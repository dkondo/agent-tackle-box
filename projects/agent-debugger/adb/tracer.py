...
class AgentTracer(bdb.Bdb, BaseCallbackHandler):
    ...
    _SKIP_PATHS = (
        ...
        "textual/",
        "rich/",
        # adb package itself (not parent dirs that happen to contain 'adb')
        "/adb/adb/",
        "adb/tracer.py",
        "adb/runner.py",
        "adb/app.py",
    ...
    def on_chain_end(
        ...
        **kwargs: Any,
        ...
        if node_name:
            ...
            # Check state-change breakpoints
            if isinstance(outputs, dict):
                if self.bp_manager.should_break_on_state(
                    self._previous_state, outputs
                ):
                    self._activate_tracing()
                self._previous_state = outputs.copy()
