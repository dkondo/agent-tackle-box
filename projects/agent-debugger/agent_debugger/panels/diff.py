...
class DiffPanel(RichLog):
    ...
    def update_diff(
        ...
        if added:
            ...
            for path, value in added.items() if isinstance(added, dict) else []:
        ...
        if removed:
            ...
            for path, value in removed.items() if isinstance(removed, dict) else []:
                val_str = str(value)[:80]
        ...
        if changed:
            ...
            for path, change in changed.items() if isinstance(changed, dict) else []:
                if isinstance(change, dict):
                    ...
                    old = "?"
                    new = str(change)[:40]
                self.write(Text(f"  ~ {path}: {old} → {new}", style="yellow"))
        ...
        for key in ("iterable_item_added", "iterable_item_removed"):
            ...
            if items:
                ...
                for path, value in items.items() if isinstance(items, dict) else []:
                    val_str = str(value)[:80]
